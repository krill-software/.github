...
**Domain-essential colors.** If your app genuinely needs colors the palette can't represent — syntax-highlighting categories, status colors on a dashboard, file-type indicators, the **image-editor's grey/white transparency checker** — declare them as app-specific named tokens (e.g. `--hl-keyword`) in the app's local `styles.css` with a comment explaining the reason. The chrome stays palette-strict; only the domain content uses these tokens. When a second krill app needs the same kind of colors, hoist the tokens into [`@krill-software/desktop-ui`](https://github.com/krill-software/desktop-ui) so all apps using that concept share the same set.
